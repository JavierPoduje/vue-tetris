...
class PieceI extends BasePiece implements Piece {
  type: PieceEnum
  coords: Set<Coord>

  constructor() {
    super()
    this.type = PieceEnum.I
    ...
    if (
      ...
      const newCoords = this._setCoords(
        [
          ...
          { row: this._sub(2), col: this._sub(2) }
        ],
        coords
      )
      ...
      const newCoords = this._setCoords(
        [
          ...
          { col: this._add(1) },
          { row: this._add(1) },
          { row: this._add(2), col: this._sub(1) }
        ],
        coords
      )
      if (!dryRun) {
      ...
      throw new Error(`Unknown lookingTo: ${this.lookingTo}`)
    }
  }
